      ...
      {data === null ? (
        ...
        >
          {rows.map((d) => (
            <figure
              key={d.id}
              className={`overflow-hidden rounded-lg border bg-surface shadow-[0_1px_2px_rgba(31,30,26,0.05)] transition hover:-translate-y-0.5 hover:shadow-[0_6px_16px_rgba(31,30,26,0.10)] motion-reduce:transition-none motion-reduce:hover:translate-y-0 ${
                d.verified
                  ? "border-border"
