...
declare module "next-auth" {
  ...
  interface Session {
    user: {
      ...
      backendToken: string;
      /** admin ตั้งรหัสให้ตอนสร้าง account — middleware บังคับไปหน้า /password ก่อน */
      mustChange: boolean;
    } & DefaultSession["user"];
  }
}
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  providers: [
    ...
    Google,
    // สำหรับเจ้าของที่ไม่มี Google — admin ตั้งรหัสให้แล้วเขามาเปลี่ยนเองที่ /password
    // ไม่เทียบรหัสที่นี่ ยิงไป backend /api/login ที่เก็บ hash (scrypt) ไว้ที่เดียว
    Credentials({
      ...
      async authorize(credentials) {
        ...
        const { token, role, mustChange } = await res.json();
        return {
  ...
  callbacks: {
    ...
    async jwt({ token, user, account, profile }) {
      if (account?.provider !== "google") {
        // credentials: มี user เฉพาะรอบที่เพิ่งล็อกอิน รอบถัดไป token มีค่าอยู่แล้ว
        if (user) {
      ...
      token.mustChange = false; // ล็อกอิน Google ไม่มีรหัสให้เปลี่ยน
      return token;
    },
    session({ session, token }) {
      session.user.role = token.role as string;
      session.user.backendToken = token.backendToken as string;
      session.user.mustChange = token.mustChange === true;
      return session;
    },
  },
});
